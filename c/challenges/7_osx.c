...
int main(int argc, char** argv) {
  ...
  uint8_t ciphertext[DEFAULT_SIZE] = "";
  char plaintext[DEFAULT_SIZE] = "";
  char base64_buf[DEFAULT_SIZE] = "";

  int n = read_file(argv[1], 1, base64_buf); 
  n = base64_decode(base64_buf, ciphertext);
  printf("n: %i\n", n);
  printf("cryptoCreate: %i\n", CCCryptorCreate(kCCDecrypt, kCCAlgorithmAES128, kCCOptionECBMode, key, kCCKeySizeAES128, &iv, &ctx));
  printf("cryptoGetOutputLength: %i\n", CCCryptorGetOutputLength(ctx, n, 1));
  printf("dataOutAvail: %i\n", dataOutAvail);
  printf("dataOutMoved: %i\n", dataOutMoved);
  printf("cryptoUpdate: %i\n", CCCryptorUpdate(ctx, ciphertext, n, plaintext, dataOutAvail, dataOutMoved));
  CCCryptorFinal(ctx, plaintext, dataOutAvail, dataOutMoved);
  printf("%s\n", plaintext);
  return 0;
